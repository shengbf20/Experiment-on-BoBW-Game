...
sqrt(T) trend would pass. It is not an independent rate proof.

This check is isolated from the separation-example generator: it must not
import ClosedFormPlayer into src/hsieh.py, and it does not read or write
Part A frozen opponent files.
"""
...
_DTYPE = np.float64
HSIEH_SRC = ROOT / "src" / "hsieh.py"


def test_module_isolation():
    tree = ast.parse(HSIEH_SRC.read_text(encoding="utf-8"))
    imported = []
    ...
    forbidden = {"learner", "separation", "ClosedFormPlayer"}
    hit = forbidden.intersection(imported)
    if hit:
        raise AssertionError(f"src/hsieh.py must stay isolated, imported {hit}")
...
def test_g2_selfplay_platforms():
    ...
    print(
        ...
        "(smoke only; not a BoBW comparison)"
    )
